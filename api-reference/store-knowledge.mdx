...
<ParamField body="tenant_id" type="string" required initialValue="YOUR_API_KEY">
...
```bash cURL
curl -X POST https://api.osmosis.ai/store_knowledge \
...
async function storeKnowledge() {
  ...
  const response = await axios.post('https://api.osmosis.ai/store_knowledge', payload, {
    headers: {
...
def store_knowledge():
    ...
    response = requests.post(
        "https://api.osmosis.ai/store_knowledge",
        json=payload,
        headers={
            "Content-Type": "application/json",
...
func storeKnowledge() ([]byte, error) {
    ...
    req, err := http.NewRequest("POST", "https://api.osmosis.ai/store_knowledge", bytes.NewBuffer(jsonData))
    if err != nil {
        return nil, err
    }
